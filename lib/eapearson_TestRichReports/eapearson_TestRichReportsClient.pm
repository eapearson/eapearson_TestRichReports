package eapearson_TestRichReports::eapearson_TestRichReportsClient;
...
eapearson_TestRichReports::eapearson_TestRichReportsClient
...
sub new
{
    ...
    my $self = {
	client => eapearson_TestRichReports::eapearson_TestRichReportsClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
sub status
{
    ...
    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
        method => "eapearson_TestRichReports.status",
        params => \@args,
    });
    if ($result) {
        if ($result->is_error) {
...
sub version {
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "eapearson_TestRichReports.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for eapearson_TestRichReports::eapearson_TestRichReportsClient\n";
    }
    if ($sMajor == 0) {
        warn "eapearson_TestRichReports::eapearson_TestRichReportsClient version is $svr_version. API subject to change.\n";
    }
}
...
package eapearson_TestRichReports::eapearson_TestRichReportsClient::RpcClient;
